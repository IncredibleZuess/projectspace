...
@export var zoomLevel = 200.0

func _input(event):
	var camera_speed = speed # Adjust for desired speed
	var zoom_speed = Vector2(0.005,0.005) # Adjust for desired zoom speed
	var zoom_min = Vector2(0.001,0.001) # Set the minimum zoom level
	var zoom_max = Vector2(zoomLevel,zoomLevel) # Set the maximum zoom level
